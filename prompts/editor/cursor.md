...
curl -sSL https://raw.githubusercontent.com/wallapi/taggbox.com-API-Docs/main/prompts/TAGGBOX_CONTEXT.md >> .cursor/rules/taggbox.mdc
```
...
(Invoke-WebRequest https://raw.githubusercontent.com/wallapi/taggbox.com-API-Docs/main/prompts/TAGGBOX_CONTEXT.md).Content | Add-Content .cursor/rules/taggbox.mdc
```
...
```
Build the Taggbox social widget described in .cursor/rules/taggbox.mdc and llms.txt in this folder. First open the theme picker from the theme catalogue in my browser - the thumbnails page itself, not a list of names - and ask which theme I want; then ask which language I want the server code in (any: Python, PHP, Node.js, Go, ...). One question per reply, and start building on my second answer.
